#include "GPUWaveApp.h"

#include "Common/GeometryGenerator.h"

const int gNumFrameResources = 3;

GPUWaveApp::GPUWaveApp(HINSTANCE hInstance)
	:D3DApp(hInstance)
{
}

GPUWaveApp::~GPUWaveApp()
{
	if (md3dDevice != nullptr)
		FlushCommandQueue();
}

bool GPUWaveApp::Initialize()
{
...
void GPUWaveApp::Update(const GameTimer& gt)
{
	OnKeyboardInput(gt);
	UpdateCamera(gt);
...
void GPUWaveApp::Draw(const GameTimer& gt)
{
...
void GPUWaveApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void GPUWaveApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void GPUWaveApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void GPUWaveApp::OnMouseScroll(WPARAM btnState, int x, int y)
{
...
void GPUWaveApp::OnKeyboardInput(const GameTimer& gt)
{
...
void GPUWaveApp::UpdateCamera(const GameTimer& gt)
{
	// 把球坐标转换为直角坐标
...
void GPUWaveApp::UpdateObjectCBs(const GameTimer& gt)
{
...
void GPUWaveApp::UpdateMainPassCB(const GameTimer& gt)
{
...
void GPUWaveApp::UpdateMaterialCBs(const GameTimer& gt)
{
...
void GPUWaveApp::UpdateWaves(const GameTimer& gt)
{
...
void GPUWaveApp::AnimateMaterials(const GameTimer& gt)
{
	// 使水流材质的纹理坐标流动起来
...
void GPUWaveApp::BulidDescriptorHeap()
{
	//
	// Create the descriptor heap.
	//
	D3D12_DESCRIPTOR_HEAP_DESC cbvsrvuavHeapDesc = {};
	cbvsrvuavHeapDesc.NumDescriptors = 3 +  mWaveSimulator->DescriptorCount();
...
void GPUWaveApp::BuildDescriptors(CD3DX12_CPU_DESCRIPTOR_HANDLE& cpuHandle, CD3DX12_GPU_DESCRIPTOR_HANDLE& gpuHandle, UINT descriptorSize)
{
	//
...
void GPUWaveApp::BuildShadersAndInputLayout()
{
...
void GPUWaveApp::BuildRootSignature()
{
	...
	CD3DX12_STATIC_SAMPLER_DESC samplers[2] = { samLinearWrap, samPointClamp };
...
void GPUWaveApp::BuildBoxGeometry()
{
...
void GPUWaveApp::BuildLandGeometry()
{
	GeometryGenerator geoGen;
...
void GPUWaveApp::BuildWavesGeometry()
{
	GeometryGenerator geoGen;
...
void GPUWaveApp::BuildMaterials()
{
...
void GPUWaveApp::BuildRenderItems()
{
...
void GPUWaveApp::BuildFrameResources()
{
...
void GPUWaveApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
...
float GPUWaveApp::GetHillsHeight(float x, float z) const
{
	return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 GPUWaveApp::GetHillsNormal(float x, float z)const
{
	// n = (-df/dx, 1, -df/dz)
	XMFLOAT3 n(
