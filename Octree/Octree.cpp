...
Octree<T>::Octree(uint32_t N_LEVELS)
	:m_Levels(N_LEVELS), m_RootLevel(N_LEVELS - 1), m_MaxValue((float)(1 << (N_LEVELS - 1))), m_CellSize(1 / m_MaxValue)
{
...
template<typename T>
uint32_t Octree<T>::LocatePoint(const DirectX::XMFLOAT3& p)
{
	...
	return LocateCell(xLocCode, yLocCode, zLocCode, 0);
}

template<typename T>
uint32_t Octree<T>::LocateRegion(const DirectX::XMFLOAT3& v_min, const DirectX::XMFLOAT3& v_max)
{
	if (OT_OUT_OF_RANGE(v_min.x) || OT_OUT_OF_RANGE(v_min.y) || OT_OUT_OF_RANGE(v_min.z))
		return OT_NIL;
	if (OT_OUT_OF_RANGE(v_max.x) || OT_OUT_OF_RANGE(v_max.y) || OT_OUT_OF_RANGE(v_max.z))
		return OT_NIL;
	...
	return LocateCell(x0LocCode, y0LocCode, z0LocCode, zLevel);
}

template<typename T>
uint32_t Octree<T>::RayCast(uint32_t curr, const DirectX::XMFLOAT3& p, const DirectX::XMFLOAT3& u)
{
...
template<typename T>
uint32_t Octree<T>::Parent(uint32_t idx)
{
...
template<typename T>
uint32_t Octree<T>::Child(uint32_t idx, uint32_t k)
{
	uint32_t level = m_Cells[idx].Level;
	if (level == 0)
		return OT_NIL;
	...
	return LocateCell(xLocCode | xChildBit, yLocCode | yChildBit, zLocCode | zChildBit, level - 1);
}

template<typename T>
void Octree<T>::CellToAABB(uint32_t idx, DirectX::XMFLOAT3& v_min, DirectX::XMFLOAT3& v_max)
{
	CellToAABB(m_Cells[idx], v_min, v_max);
}

template<typename T>
void Octree<T>::CellToAABB(const Cell& cell, DirectX::XMFLOAT3& v_min, DirectX::XMFLOAT3& v_max)
{
...
template<typename T>
T& Octree<T>::ReceiveData(uint32_t idx)
{
	return m_Cells[idx].data;
}

template<typename T>
void Octree<T>::EnCode()
{
...
template<typename T>
uint32_t Octree<T>::LocateNeighbor(uint32_t idx, int x, int y, int z)
{
	uint32_t xLocCode = m_Cells[idx].XLocCode;
	uint32_t yLocCode = m_Cells[idx].YLocCode;
	uint32_t zLocCode = m_Cells[idx].ZLocCode;
	uint32_t level = m_Cells[idx].Level;
	uint32_t levelMask = ~((1 << level) - 1);
	...
	uint32_t xNeighborLocCode = (xLocCode + x) & levelMask;
	uint32_t yNeighborLocCode = (yLocCode + y) & levelMask;
	uint32_t zNeighborLocCode = (zLocCode + z) & levelMask;

	return LocateCell(xNeighborLocCode, yNeighborLocCode, zNeighborLocCode, level);
}

template<typename T>
uint32_t Octree<T>::LocateCell(uint32_t xLocCode, uint32_t yLocCode, uint32_t zLocCode, uint32_t level)
{
...
template<typename T>
float Octree<T>::TimeToEscape(float v_m, float v_M, float p, float u)
{
	float v = abs(u);
	if (v < FLT_EPSILON)
		return FLT_MAX;
